...
router.post('/', function(req, res, next) {
	...
	if (session.userId) {
		if (req.body.sessionCode &&
			...
			const debate = asm.get(req.body.sessionCode);
			if (debate) {
				if (debate.getModeratorId() !== session.userId) {
					let debateRole = session.activeDebates[req.body.sessionCode];
					...
					if (!voter) { // if they don't, add them to the session
						voter = debate.addVoter(req.body.sessionCode, req.body.name, debateRole.voterId);
